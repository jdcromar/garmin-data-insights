...
def fetch_activities(client: Garmin, start: date, end: date,
    ...
    with get_conn() as conn:
        existing = set() if force else {
            row[0] for row in conn.execute("SELECT activity_id FROM activities").fetchall()
        }
        for a in activities:
...
def fetch_daily_stats(client: Garmin, start: date, end: date,
                      on_progress: Optional[Callable] = None, force: bool = False):
    """Fetch daily step/calorie stats for a date range."""
    current = start
    with get_conn() as conn:
        existing = set() if force else {
            row[0] for row in conn.execute("SELECT date FROM daily_stats").fetchall()
        }
        while current <= end:
...
def fetch_sleep(client: Garmin, start: date, end: date,
                on_progress: Optional[Callable] = None, force: bool = False):
    """Fetch sleep data for a date range."""
    current = start
    with get_conn() as conn:
        existing = set() if force else {
            row[0] for row in conn.execute("SELECT date FROM sleep").fetchall()
        }
        while current <= end:
            day = current.isoformat()
            if day not in existing:
...
def fetch_hrv(client: Garmin, start: date, end: date,
              on_progress: Optional[Callable] = None, force: bool = False):
    """Fetch HRV data for a date range."""
    current = start
    with get_conn() as conn:
        existing = set() if force else {
            row[0] for row in conn.execute("SELECT date FROM hrv").fetchall()
        }
        while current <= end:
            day = current.isoformat()
            if day not in existing:
...
def fetch_body_battery(client: Garmin, start: date, end: date,
                       on_progress: Optional[Callable] = None, force: bool = False):
    """Fetch body battery data for a date range."""
    current = start
    with get_conn() as conn:
        existing = set() if force else {
            row[0] for row in conn.execute("SELECT date FROM body_battery").fetchall()
        }
        while current <= end:
            day = current.isoformat()
            if day not in existing:
...
def sync_all(client: Garmin, start: date, end: date,
             on_progress: Optional[Callable] = None, force: bool = False):
    """Sync all data types for the given range."""
    fetch_activities(client, start, end, on_progress, force)
    fetch_daily_stats(client, start, end, on_progress, force)
    fetch_sleep(client, start, end, on_progress, force)
    fetch_hrv(client, start, end, on_progress, force)
    fetch_body_battery(client, start, end, on_progress, force)
